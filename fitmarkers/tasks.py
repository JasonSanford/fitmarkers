...
def get_new_mmf_workouts(social_auth_users, since_date):
    for sau in social_auth_users:
        ...
        activities_response = mmf_api.get('/v7.0/workout/?{0}'.format(encoded_params))
        logger.info('activities_response: %s' % activities_response.json())
        #raw_workouts = activities_response.json()['items']
        #logger.info('raw_workouts: %s' % raw_workouts)
        """
        existing_workout_count = 0
        no_path_workout_count = 0
        workouts_to_create = {}
